...
function createJSONAPIRouter(settings) {
    ...
    for (const type of Object.keys(schema.models)) {
        ...
        router.get(`findRecords(${type})`, resourcePath, async (ctx) => {
            ...
            if (filter) {
                term.filter(...deserialize_params_1.deserializeFilterParams(source.schema, serializer.resourceFieldParamSerializer(), type, filter));
            }
            if (sort) {
                term.sort(...deserialize_params_1.deserializeSortParams(schema, serializer.resourceFieldParamSerializer(), type, sort));
            }
        ...
        schema.eachRelationship(type, (propertyName, { kind, type: relationshipType }) => {
            ...
            if (kind === 'hasMany') {
                router.get(`findRelatedRecords(${type}, ${relationshipType})`, relationshipPath, async (ctx) => {
                    ...
                    if (filter) {
                        term.filter(...deserialize_params_1.deserializeFilterParams(source.schema, serializer.resourceFieldParamSerializer(), relationshipType, filter));
                    }
                    if (sort) {
                        term.sort(...deserialize_params_1.deserializeSortParams(schema, serializer.resourceFieldParamSerializer(), relationshipType, sort));
                    }
                    const query = data_1.buildQuery(term, {
                        from: 'jsonapi',
                        [name]: { headers, include },
